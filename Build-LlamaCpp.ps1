...
.DESCRIPTION
    ...
    4. Test with your specialist model + MCP.
    ...
    - Applies GodBrain overlays (godbrain_chat_extensions.cpp etc.) from .\llama-overrides before configure.
...
function Write-BuildWarnings {
    $warn = [string]::Join([Environment]::NewLine, @(
        ...
        "",
        "  The GodBrain overlays (godbrain_chat_extensions.cpp + tokens for write_local_file,",
        "  list_local_dir, local_sovereign_fs, c_drive_sovereignty, bypass_browser_sandbox etc.)",
        "  + the automatic wiring into chat.cpp (the calls to inject/apply) happen before",
        "  configure. Once you have a clean successful build, your specialist will have",
        "  native bias for the privileged C: write path (Python MCP does the real IO with",
        "  full rights  browser storage rules / Sec-Fetch / Activate-Storage-Access are",
        "  completely bypassed, as they should be on the PC God node)."
    ))
    Write-Color $warn "Yellow"
}

function Connect-GodBrainChatExtensions {
    $chatCpp = Join-Path $SourceDir "common\chat.cpp"
    if (-not (Test-Path $chatCpp)) {
        Write-Color "    (no common/chat.cpp yet  wiring skipped)" DarkGray
        return
    }

    $content = Get-Content -LiteralPath $chatCpp -Raw -ErrorAction Stop

    if ($content -match "godbrain_chat_extensions|apply_godbrain_chat_extensions|inject_godbrain_preserved_tokens") {
        Write-Color "    GodBrain extensions already referenced in chat.cpp" Green
        return
    }

    Write-Color "[*] Wiring GodBrain chat extensions (preserved tokens for MCP tools incl. write_local_file)..." Cyan

    # 1. Add the include right after the main chat.h include (additive, safe)
    $includeMarker = '#include "chat.h"'
    if ($content -match [regex]::Escape($includeMarker)) {
        $content = $content -replace ([regex]::Escape($includeMarker)),
            ($includeMarker + "`r`n#include `"godbrain_chat_extensions.cpp`"  // GodBrain: preserved_tokens + specialist bias (write_local_file, local_sovereign_fs, etc.)")
    } else {
        $content = "#include `"godbrain_chat_extensions.cpp`"`r`n" + $content
    }

    # 2. Insert the actual calls in the most important init (gemma4 / specialist path).
    # We look for the gemma4 params init and inject before a typical "return data;" or end of function.
    # This is string-based and may need a one-time manual tweak if the upstream layout changed a lot.
    $gemmaPattern = 'common_chat_params common_chat_params_init_gemma4'
    if ($content -match $gemmaPattern) {
        $insert = [string]::Join([Environment]::NewLine, @(
            "",
            "    // === GodBrain additive wiring (do not remove) ===",
            "    // Ensures the 26B specialist (and any `"GodBrain`"/gemma-4-26B model) gets our",
            "    // preserved tokens for save_godbrain_thought, query_constellation, write_local_file,",
            "    // list_local_dir, local_privileged_fs protocol, c_drive_sovereignty, etc.",
            "    // Also applies the architect-mode comments / goal-directed bias.",
            "    godbrain::inject_godbrain_preserved_tokens(data);",
            "    godbrain::apply_godbrain_chat_extensions(data, `"gemma-4-26B-GodBrain`");",
            "    // === end GodBrain wiring ===",
            ""
        ))
        # Better targeted insert: find the gemma init function and insert before its last return data/params;
        # Use a non-greedy match up to the return inside that function (assumes no deep nesting or we take the last one).
        $pattern = '(?s)(common_chat_params common_chat_params_init_gemma4\s*\([^)]*\)\s*\{.*?)(return\s+(data|params)\s*;\s*\})'
        if ($content -match $pattern) {
            $content = $content -replace $pattern, ('$1' + $insert + '$2')
        } else {
            # Fallback: append the block just before the final } of the file if nothing matched (rare)
            $content = $content -replace '(\s*\}\s*)$', ($insert + '$1')
        }
    }

    Set-Content -LiteralPath $chatCpp -Value $content -NoNewline
    Write-Color "    Wired. The next successful build will give the specialist native knowledge of write_local_file + the new FS sovereignty tokens." Green
    Write-Color "    (If the insert point was off, open common/chat.cpp in the source and move the two godbrain:: calls to right before the final 'return data;' inside the gemma4 init.)" DarkGray
}
...
function Deploy-Artifacts {
    ...
    $buildInfo = [ordered]@{
        ...
        notes = "Custom build for GodBrain specialist (Gemma-4 26B) + MCP tool calling. Patches live in GodBrain/llama-overrides/"
    }
    ...
    $infoString = [string]::Join([Environment]::NewLine, @(
        ...
        "Notes: Custom build for GodBrain specialist + MCP tool calling.",
